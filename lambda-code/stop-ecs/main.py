...
# ---------- ฟังก์ชันเรียก stop ECS ----------
def stop_ecs_instance(client, instance_id):
    """
    ใช้ BatchStopServers เพื่อปิดเครื่อง
    stop_type:
      - "SOFT" = ส่ง shutdown ปกติ (แนะนำ)
      - "HARD" = ตัดเครื่องทันทีเหมือนดึงปลั๊ก
    """
    req_body = BatchStopServersRequestBody(
        os_stop=BatchStopServersOption(
            servers=[ServerId(id=instance_id)],
            type="SOFT"   # หรือ "HARD" ถ้าต้องการบังคับ
        )
    )
    request = BatchStopServersRequest(body=req_body)
    response = client.batch_stop_servers(request)
    print(f"เรียก BatchStopServers (SOFT) สำหรับ {instance_id} แล้ว")
    return response
...
def list_ecs_by_tag_value(client, tags):
    ...
    for s in servers:
        ...
        if schedule_match and env_match:
            print(
                f"Matched ECS: id={s.id}, name={getattr(s, 'name', '')}, "
                f"metadata={metadata}, tags={tags_list}"
            )
            instance_ids.append(s.id)
        else:
            print(
                f"Skip ECS: id={s.id}, name={getattr(s, 'name', '')}, "
                f"metadata={metadata}, tags={tags_list}"
            )
...
def stop_ecs_all():
    ...
    if len(ids) > 0:
        for instance_id in ids:
            ...
            stop_ecs_instance(client, instance_id)
    else:
